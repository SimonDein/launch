...
function textSentiment(text) {
  let sentiments = getSentiments(text);
  let sentiment = getSentiment(sentiments);
  
  console.log(`There are ${sentiments.positive.length} positive words in the text.`);
  console.log(`Positive sentiments: ${sentiments.positive.join(', ')}`)
  console.log('')
  console.log(`There are ${sentiments.negative.length} negative words in the text.`);
  console.log(`Positive sentiments: ${sentiments.negative.join(', ')}`)
  console.log('')
...
function getSentiments(text) {
  ...
  positiveWords.forEach(function (word) {
    matches = text.match(RegExp(`\\b${word}\\b`, 'ig'));
    if (matches === null) return;
    sentiments.positive = sentiments.positive.concat(matches);
  });

  negativeWords.forEach(function (word) {
    matches = text.match(RegExp(`\\b${word}\\b`, 'ig'));
    if (matches === null) return;
    sentiments.negative = sentiments.negative.concat(matches);
  });

  return sentiments;
}

function getSentiment(sentiments) {
  let sentimentScore = sentiments.positive.length - sentiments.negative.length;
  let sentiment;
