...
function mostWordsSentence(text) {
  ...
  let mostWordsInASentence = sentences.reduce(function (maxWordCount, sentence, index) {
    wordcount = sentence.split(' ').length;
    if (wordcount > maxWordCount) {
      sentenceIndex = index;
      return wordcount
    } else return maxWordCount;
  }, 0);

  console.log(sentences[sentenceIndex])
  console.log(`The longest sentence has ${mostWordsInASentence} words`);
}

mostWordsSentence(longText);
