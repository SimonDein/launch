...
function hasIdAndTitle(releaseData) {
  let headers = Object.keys(releaseData);
  return headers.includes('id') && headers.includes('title');
}

function processReleaseData(releases) {
  let releasesWithIdAndTitle = releases.filter(hasIdAndTitle);
  
  return releasesWithIdAndTitle.map(function (releaseData) {
    return {id: releaseData['id'], title: releaseData['title']}
  });
}
