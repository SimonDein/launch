=begin
Description:

Write a program that can calculate the Hamming difference between two DNA strands.

A mutation is simply a mistake that occurs during the creation or copying of a nucleic acid, in particular DNA
Because nucleic acids are vital to cellular functions, mutations tend to cause a ripple effect throughout the cell
Although mutations are technically mistakes, a very rare mutation may equip the cell with a beneficial attribute.
In fact, the macro effects of evolution are attributable by the accumulated result of beneficial microscopic mutations over many generations.

The simplest and most common type of nucleic acid mutation is a point mutation, which replaces one base with another at a single nucleotide.

By counting the number of differences between two homologous DNA strands taken from different genomes with a common ancestor,
we get a measure of the minimum number of point mutations that could have occurred on the evolutionary path between the two strands.

This is called the 'Hamming distance'

GAGCCTACTAACGGGAT
CATCGTAATGACGGCCT
^ ^ ^  ^ ^    ^^
The Hamming distance between these two DNA strands is 7.

The Hamming distance is only defined for sequences of equal length.
If you have two sequences of unequal length, you should compute the Hamming distance over the shorter length.

Breaking down the problem:

Input:
  -String
Output:
  -Integer

Data Structure
  -Some sort of iteration could be beneficial
  -In the end we only need a number - meaning we have to count
  -Array#reduce could do both
  -I will start with a solution working with an array
=end

# solution 1
class DNA
  attr_reader :original_dna
  
  def initialize(original_dna)
    @original_dna = original_dna
  end1  

  def hamming_distance(mutated_dna)
    hamming_distance = 0
    mutated_dna.each_char.with_index do |dna, index|
      break unless original_dna[index] # if we have two DNA-strands of unequal length we compute the distance over the shorter length
      hamming_distance += 1 unless dna == original_dna[index] # accumulate if we find a difference
    end
    hamming_distance
  end
end

# solution 2

class DNA
  attr_reader :original_dna
  
  def initialize(original_dna)
    @original_dna = original_dna
  end

  def hamming_distance(mutated_dna)
    length = [original_dna.size, mutated_dna.size].min - 1
    (0..length).reduce(0) do |distance, index|
      original_dna[index] == mutated_dna[index] ? distance : distance += 1
    end
  end
end