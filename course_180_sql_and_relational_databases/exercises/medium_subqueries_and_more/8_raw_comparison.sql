...
Write an SQL query that will display the id for the item that matches all of the data that we know, but does not use the AND keyword.
...
SELECT items.id
 ...
 WHERE ROW('Painting', 100.00, 250.00) =
       ROW(items.name, items.initial_price, sales_price);
